...
- `options`: Optional ordering and pagination parameters
  - `continuationToken`: Token for paginating through results
  - `limit`: Maximum number of entries to return
  - `sortBy`: Field to order by: `"entityId"`, `"entityType"`, `"clientId"`, `"actionType"`, or `"created"` (defaults to `"created"`)
  - `sortDirection`: Order direction, either `"asc"` or `"desc"` (defaults to `"desc"`)
